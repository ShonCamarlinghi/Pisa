...
outDir = 'C:\Users\scamarlinghi\Documents\CVQ\CVQ_Dragonboard'
FW = '51747'
VP = 'VPon'
Delay = '2000ms'
...
CurrentDeviceX = int(device.getProperty("display.width"))
CurrentDeviceY = int(device.getProperty("display.height"))
...
def CVQ_preset():
	device.shell('logcat -c')
	time.sleep(1)
	device.touch(transX(160),transY(820),MonkeyDevice.DOWN_AND_UP)
	while True:
...
def pull_flac(outDirComb,outDirFlac, b):
	print "Starting pull to %s" % outDir
	pattern = re.compile('|'.join(re.escape(key) for key in d.keys()))
	b = pattern.sub(lambda x: d[x.group()], b)
	flacPath = '%s\%s.flac' % (outDirFlac, b)
	combPath = '%s\%s.pcm' % (outDirComb, b)
...
def snooz():
	...
	device.touch(transX(460),transY(900),MonkeyDevice.DOWN_AND_UP)
	time.sleep(1)
	CVQ_preset()
...
def KWcount(value, outDir):
	...
	outDirFlac = '%s\Flac' % outDir
	outDirComb = '%s\Comb' % outDir
	init_state()
	...
	while True:
		...
		try:
			if 'detected cvs event' in logcat0:
				...
				asr(ASRres, b)
				pull_flac(outDirComb,outDirFlac, b)
